 ...
 *
 *  LG WebOS 3.0 TV Service Manager
 *
 *  Author: Roro Ultimum
 */
 
definition(
	name: "LG WebOS 3.0 TV (Connect)",
	namespace: "rrjjbb",
	author: "Roro Ultimum",
	description: "Allows you to control your LG WebOS 3.0 TV from the SmartThings app. Perform basic functions like power Off, source, volume, channels and other remote control functions.",
	category: "Convenience",
    iconUrl: "https://lh3.googleusercontent.com/c3JN13-iQcao3JxAXYih03dYwm63COL-Bh3aD2MswYdUguHmYjDNEo_3VawQuZPx4acx=w300",
    iconX2Url: "https://lh3.googleusercontent.com/c3JN13-iQcao3JxAXYih03dYwm63COL-Bh3aD2MswYdUguHmYjDNEo_3VawQuZPx4acx=w300",
    singleInstance: true
)

preferences {
	page(name:"lgDiscovery", title:"LG WebOS 3.0 TV Setup", content:"LGDiscovery", refreshTimeout:5)
}
...
def LGDiscovery()
{
	if(canInstallLabs())
	{	
		int lgRefreshCount = !state.lgRefreshCount ? 0 : state.lgRefreshCount as int
		state.lgRefreshCount = lgRefreshCount + 1
		...
		if((lgRefreshCount % 5) == 0) {
        	log.trace "Discovering..."
			discoverLGs()
		}

		//setup.xml request every 3 seconds except on discoveries
		if(((lgRefreshCount % 1) == 0) && ((lgRefreshCount % 8) != 0)) {
            log.trace "Verifing..."
			verifysamsungPlayer()
		}

		return dynamicPage(name:"lgDiscovery", title:"Discovery Started!", nextPage:"", refreshInterval:refreshInterval, install:true, uninstall: true) {
			section("Please wait while we discover your LG TV. Discovery can take five minutes or more, so sit back and relax! Select your device below once discovered.") {
				input "selectedsamsung", "enum", required:false, title:"Select LG WebOS 3.0 TV (${numFound} found)", multiple:true, options:options
			}
		}
	}
	else
	{
		def upgradeNeeded = """To use this SmartApp, your Hub should be completely up to date.

To update your Hub, access Location Settings in the Main Menu (tap the gear next to your location name), select your Hub, and choose "Update Hub"."""

		return dynamicPage(name:"lgDiscovery", title:"Upgrade needed!", nextPage:"", install:true, uninstall: true) {
			section("Upgrade") {
...
def uninstalled() {
	...
	log.trace "deleting ${devices.size()} LG"
	devices.each {
...
def addsamsung() {
	...
	selectedsamsung.each { dni ->
		...
		if(!d) {
			...
			log.trace "dni = $dni"
			d = addChildDevice("rrjjbb", "LG WebOS 3.0 Smart TV", dni, newPlayer?.value.hub, [label:"${newPlayer?.value.name}"])
...
private verifysamsungPlayer() {
	def devices = getLGPlayer().findAll { it?.value?.verified != true }
...
{
	getLGPlayer().findAll{ it?.value?.verified == true }
}
